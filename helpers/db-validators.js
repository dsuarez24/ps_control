...
const existeAlumnoById = async (id = '') => {
    ...
    if (existeAlumno) {
        throw new Error(`Este Alumno con el ${id} no existe`);
    }
}
...
const existeProfesorById = async (id = '') => {
    ...
    if (existeProfesor) {
        throw new Error(`Este Profesor con el ${id} no existe`);
    }
}
...
const esCursoValido = async (cursos = []) => {
    for (const id of cursos) {
        if (!mongoose.Types.ObjectId.isValid(id)) {
            throw new Error(`Este id '${id}'  no es un Id válido`);
        }

        const cursoExistente = await Cursos.findOne({ _id: id });

        if (!cursoExistente) {
            throw new Error(`Este curso con el id: ${id} no existe en la base de datos`);
        }
    }
};
